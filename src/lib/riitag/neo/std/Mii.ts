...
import { IImageBackground } from "./Types";
...
export default class Mii extends ModuleBase {
    ...
    render(ctx: Canvas.CanvasRenderingContext2D, user): any {
        if (!user.show_mii) { this.events.emit("rendered"); return; }
        if (!this.enabled) { this.events.emit("rendered"); return; }

        this.getMii(user).then((mii) => {
            if (this.background) {
